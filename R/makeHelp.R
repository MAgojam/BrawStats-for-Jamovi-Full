makeHelpOut<-function(brawHelp,statusStore) {
  ...
  if (brawHelp) {
    ...
    metaHelp<-brawMetaHelp(open=statusStore$metaHelpWhich,indent,titleWidth)
    ...
    if (statusStore$nestedHelp) {
      open0<-max(0,statusStore$basicHelpWhich,any(statusStore$demoHelpWhich>0)*2,(statusStore$metaHelpWhich>0)*3,(statusStore$simHelpWhich>0)*4,(statusStore$openJamovi>0)*5)
      help<-generate_tab(
        title="Help:",
        plainTabs=TRUE,
        titleWidth=50,
        tabs=c("Basic","Demos","Investigations","Simulations","Jamovi","Key"),
        tabContents=c(basicHelp,demoHelp,metaHelp,simHelp,jamoviHelp,BrawInstructions("Key")),
        open=open0
      )
    } else help<-paste0(basicHelp,metaHelp,demoHelp,simHelp,jamoviHelp) 
  } else help<-''
